...
sub start_deployment {
  ...
  my $payload = {deploy_state => 'pending', deploy_user => $user, deploy_environment => $self->{config}->{"environment"}, deployd_before => $push_request->{"before"}};
  my $params = {ref => $branch, auto_merge => Mojo::JSON->false, payload => $payload, description => "Deploying to production server", environment => $self->{config}->{"environment"}};
